...
async function authenticatedActor(req: Request, actorKey: string): Promise<{ ok: boolean; message?: string }> {
  ...
  if (linkedAuthUser && linkedAuthUser !== data.user.id) {
    return { ok: false, message: "Sesion no pertenece al usuario que solicita reporte." };
  }
  return { ok: true };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const actorKey = clean(body.actorKey ?? body.supervisorKey);
    ...
    const { data, error } = await supabase.rpc("ln_legacy_report", { p_body: body });
