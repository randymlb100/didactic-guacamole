...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!adminId || !payload || typeof payload !== "object") {
      return json({ ok: false, message: "adminId y payload son requeridos." }, 400);
    }
    const configKey = `cashier_limits:${adminId}`;
    ...
    return json({ ok: false, message: error instanceof Error ? error.message : "No se pudo guardar cashier limits." }, 500);
  }
});
