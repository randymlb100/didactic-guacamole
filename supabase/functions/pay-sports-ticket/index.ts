...
const PAY_ROLES = new Set(["admin", "cashier"]);
...
async function handle(req: Request): Promise<Response> {
  ...
  if (!PAY_ROLES.has(actorRole)) return json({ ok: false, message: "Este rol no puede pagar cobros deportivos." }, 403);
  if (!metadataMatchesActor(auth.metadata, actorKey, adminKey, cashierKey)) {
    return json({ ok: false, message: "Sesion no pertenece al cobro deportivo." }, 403);
  }
  ...
  if (previousStatus === "paid") {
    return json({ ok: true, alreadyPaid: true, ticket: ticketPayload(ticket as JsonMap) });
  }
...
Deno.serve((req) => handle(req).catch((error) => {
  return json({ ok: false, message: error instanceof Error ? error.message : "Error pagando ticket deportivo." }, 500);
}));
