-- Guard the v2 reconcile queue so cache backfills and repeated refreshes do not
-- create a large pile of duplicate jobs before the worker is enabled.

update public.result_reconcile_jobs
set status = 'deferred_backfill',
    completed_at = coalesce(completed_at, now()),
    last_error = 'Deferred by v2 queue guard after initial backfill validation.'
where status = 'pending';

create unique index if not exists result_reconcile_jobs_one_active_idx
on public.result_reconcile_jobs (
  result_day_key,
  coalesce(lottery_legacy_id, ''),
  coalesce(game, '')
)
where status in ('pending', 'running');

create or replace function public.lotterynet_enqueue_result_reconcile_job(
...
begin
  ...
  values (p_result_day_key, p_lottery_legacy_id, p_game)
...
revoke all on function public.lotterynet_enqueue_result_reconcile_job(text, text, text) from public, anon, authenticated;
