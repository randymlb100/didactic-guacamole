...
drop trigger if exists ln_protect_ticket_owner_snapshot_trigger on public.lotterynet_tickets_by_owner;
create trigger ln_protect_ticket_owner_snapshot_trigger
before insert or update of payload on public.lotterynet_tickets_by_owner
for each row
execute function public.ln_protect_ticket_owner_snapshot();

revoke all on function public.ln_protect_ticket_owner_snapshot() from public, anon, authenticated;
