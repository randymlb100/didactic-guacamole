create or replace function public.lotterynet_quiniela_hit_positions(
  digits text,
  first_pick text,
  second_pick text,
  third_pick text
...
as $$
  select array_remove(array[
    case when digits = first_pick then '1' end,
    case when digits = second_pick then '2' end,
    case when digits = third_pick then '3' end
  ], null)
$$;

create or replace function public.lotterynet_resolve_ticket_prize_multi_hit(ticket jsonb, result_payload jsonb)
returns jsonb
language plpgsql
stable
set search_path = public
...
begin
  ...
  for item in select value from jsonb_array_elements(coalesce(ticket->'items', '[]'::jsonb)) loop
    ...
    if play_type in ('Q','QUINIELA') then
      matched_positions := public.lotterynet_quiniela_hit_positions(digits, first_pick, second_pick, third_pick);
      if coalesce(array_length(matched_positions, 1), 0) > 0 then
        item_payout :=
          (case when digits = first_pick then amount * q1 else 0 end) +
          (case when digits = second_pick then amount * q2 else 0 end) +
          (case when digits = third_pick then amount * q3 else 0 end);
        hit_position := array_to_string(matched_positions, ',');
      end if;
...
as $$
begin
  return public.lotterynet_resolve_ticket_prize_multi_hit(ticket, result_payload);
end;
$$;
...
revoke all on function public.lotterynet_quiniela_hit_positions(text, text, text, text) from public, anon, authenticated;
revoke all on function public.lotterynet_resolve_ticket_prize_multi_hit(jsonb, jsonb) from public, anon, authenticated;
revoke all on function public.lotterynet_resolve_ticket_prize_against_payload(jsonb, jsonb) from public, anon, authenticated;
revoke all on function public.lotterynet_resolve_ticket_prize(jsonb) from public, anon, authenticated;

grant execute on function public.lotterynet_quiniela_hit_positions(text, text, text, text) to service_role;
grant execute on function public.lotterynet_resolve_ticket_prize_multi_hit(jsonb, jsonb) to service_role;
grant execute on function public.lotterynet_resolve_ticket_prize_against_payload(jsonb, jsonb) to service_role;
grant execute on function public.lotterynet_resolve_ticket_prize(jsonb) to service_role;
