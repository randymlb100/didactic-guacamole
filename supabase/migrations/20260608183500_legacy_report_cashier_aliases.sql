create or replace function public.ln_legacy_actor_aliases(p_actor_key text)
returns text[]
language plpgsql
stable
security definer
...
declare
  ...
  v_aliases text[];
begin
  v_actor := public.ln_actor_from_legacy_state(p_actor_key);

  select coalesce(array_agg(distinct alias), array[]::text[])
    into v_aliases
  from unnest(array[
    lower(trim(coalesce(p_actor_key, ''))),
    ...
    lower(trim(coalesce(v_actor ->> 'cashierId', ''))),
    lower(trim(coalesce(v_actor ->> 'cashierUser', ''))),
    lower(trim(coalesce(v_actor ->> 'cashierKey', ''))),
    lower(trim(coalesce(v_actor ->> 'authUserId', ''))),
    lower(trim(coalesce(v_actor ->> 'auth_user_id', '')))
  ]) alias
  where alias <> '';
...
revoke all on function public.ln_legacy_actor_aliases(text) from public, anon, authenticated;
grant execute on function public.ln_legacy_actor_aliases(text) to service_role;
