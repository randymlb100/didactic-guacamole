...
create or replace function public.lotterynet_owner_payload_ticket_matches(ticket jsonb, ticket_row public.tickets)
returns boolean
language sql
stable
...
as $$
  select
    ticket->>'id' = ticket_row.id::text
    or (nullif(ticket_row.legacy_ticket_id, '') is not null and ticket->>'id' = ticket_row.legacy_ticket_id)
    or (nullif(ticket_row.client_request_id, '') is not null and ticket->>'id' = ticket_row.client_request_id)
    or (nullif(ticket_row.client_request_id, '') is not null and ticket->>'clientRequestId' = ticket_row.client_request_id)
    or (nullif(ticket_row.client_request_id, '') is not null and ticket->>'client_request_id' = ticket_row.client_request_id)
    or (nullif(ticket_row.legacy_ticket_id, '') is not null and ticket->>'legacyTicketId' = ticket_row.legacy_ticket_id)
    or (nullif(ticket_row.ticket_code, '') is not null and ticket->>'serial' = ticket_row.ticket_code)
    or (nullif(ticket_row.ticket_code, '') is not null and ticket->>'ticketCode' = ticket_row.ticket_code)
    or (nullif(ticket_row.ticket_code, '') is not null and ticket->>'ticket_code' = ticket_row.ticket_code)
$$;
...
begin
  ...
  order by
    case when owner_row.owner_key = any(owner_keys) then 0 else 1 end,
    owner_row.updated_at desc
  limit 1;
  ...
  if coalesce(array_length(owner_keys, 1), 0) > 0 then
    update public.lotterynet_tickets_by_owner owner_row
    set payload = jsonb_set(
          owner_row.payload,
          '{tickets}',
          (
            select coalesce(jsonb_agg(ticket order by ord), '[]'::jsonb)
            from jsonb_array_elements(coalesce(owner_row.payload->'tickets', '[]'::jsonb)) with ordinality as payload_ticket(ticket, ord)
            where not public.lotterynet_owner_payload_ticket_matches(ticket, ticket_row)
          ),
          true
        ),
        updated_at = now()
...
revoke all on function public.lotterynet_owner_payload_ticket_matches(jsonb, public.tickets) from public, anon, authenticated;
grant execute on function public.lotterynet_owner_payload_ticket_matches(jsonb, public.tickets) to service_role;

comment on function public.lotterynet_sync_ticket_owner_payload(uuid)
is 'Synchronizes authoritative ticket prize status into the selling snapshots, restores missing owner aliases, and prunes duplicate copies from unrelated owner snapshots.';
