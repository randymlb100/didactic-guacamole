...
begin
  ...
  candidate_keys as (
    ...
    union
    select nullif(trim(u->>'id'), '') from user_rows
    union
    select nullif(trim(u->>'user'), '') from user_rows
    union
    select nullif(trim(u->>'username'), '') from user_rows
    union
    select nullif(trim(u->>'adminId'), '') from user_rows
    union
    select nullif(trim(u->>'adminUser'), '') from user_rows
  )
...
is 'Synchronizes authoritative ticket prize status into admin/cashier owner snapshots using canonical ids and username aliases without scanning every owner payload.';
