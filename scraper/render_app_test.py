...
class RenderAppContractsTest(unittest.TestCase):
    ...
    def test_live_lottery_results_refreshes_instead_of_returning_stale_snapshot(self):
        ...
        with patch("app.get_dr_date_str", return_value="20-05-2026"), \
    ...
    def test_live_pick_results_uses_cache_without_background_scrape(self):
        ...
        with patch("app.get_dr_date_str", return_value="20-05-2026"), \
                patch("app.fetch_pick_rows_from_supabase", return_value=cached_picks), \
                patch("app.scrape_us_picks") as scrape_picks, \
                patch("app.schedule_background_pick_refresh") as schedule_pick:
            response = self.client.get("/system-results?date=20-05-2026&mode=pick&live=1")

        scrape_picks.assert_not_called()
        schedule_pick.assert_not_called()
        payload = json.loads(response.data.decode("utf-8"))
    ...
    def test_fetch_pick_rows_accepts_supabase_results_object_cache(self):
        class FakeResponse:
            def read(self):
                return json.dumps([{
                    "value": json.dumps({
        ...
        with patch("app.SUPABASE_KEY", "test-key"), \
                patch("app.urllib.request.urlopen", return_value=FakeResponse()):
            rows = app.fetch_pick_rows_from_supabase("20-05-2026")

        self.assertEqual(["19"], [row["id"] for row in rows])
