...
for (const file of files) {
  ...
  if (content.includes('voidTicket') || content.includes('deleteTicket') || content.includes('cancelTicket')) {
    console.log(`\nFile: ${file}`);
    const lines = content.split('\n');
    lines.forEach((line, idx) => {
      if (line.includes('voidTicket') || line.includes('deleteTicket') || line.includes('cancelTicket')) {
