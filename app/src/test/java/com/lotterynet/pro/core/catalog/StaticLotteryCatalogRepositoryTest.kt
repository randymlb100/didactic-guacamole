...
class StaticLotteryCatalogRepositoryTest {
    ...
    fun `catalog keeps EnLoteria aligned evening draw times`() {
        ...
        val quinielaReal = catalog.getLotteryById("5") ?: error("Missing Quiniela Real")
        ...
        assertEquals("8:55 PM", leidsa.baseDrawTime)
        assertEquals("20:55", leidsa.baseCloseTime)
        assertEquals("3:50 PM", leidsa.sundayOverride?.drawTime)
        assertEquals("15:50", leidsa.sundayOverride?.closeTime)
        assertEquals("12:55 PM", quinielaReal.baseDrawTime)
        assertEquals("12:55", quinielaReal.baseCloseTime)
